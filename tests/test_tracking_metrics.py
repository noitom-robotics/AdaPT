...
from mjlab.tasks.tracking.mdp.metrics import (
  ...
  compute_yaw_aligned_root_relative_mpkpe,
)
...
def test_yaw_aligned_mpkpe_matches_mpkpe_when_anchors_aligned(mock_command):
  """Yaw-aligned R-MPKPE matches MPKPE when anchors coincide (same alignment math)."""
  num_bodies = len(mock_command.cfg.body_names)
  identity = torch.tensor([1.0, 0.0, 0.0, 0.0])

  mock_command.anchor_pos_w = torch.zeros(mock_command.num_envs, 3)
  mock_command.robot_anchor_pos_w = torch.zeros(mock_command.num_envs, 3)
  mock_command.anchor_quat_w = identity.expand(mock_command.num_envs, 4).clone()
  mock_command.robot_anchor_quat_w = identity.expand(mock_command.num_envs, 4).clone()
  mock_command.body_pos_w = torch.rand(mock_command.num_envs, num_bodies, 3)
  mock_command.robot_body_pos_w = mock_command.body_pos_w + 0.1

  mock_command.body_pos_relative_w = (
    mock_command.body_pos_w - mock_command.anchor_pos_w.unsqueeze(1)
  )

  mpkpe = compute_mpkpe(mock_command)
  yaw_aligned = compute_yaw_aligned_root_relative_mpkpe(mock_command)

  assert torch.allclose(mpkpe, yaw_aligned, atol=1e-5)


def test_yaw_aligned_mpkpe_invariant_to_global_yaw(mock_command):
  """Yaw-aligned error is unchanged under a common global yaw rotation."""
  num_bodies = len(mock_command.cfg.body_names)
  identity = torch.tensor([1.0, 0.0, 0.0, 0.0])
  yaw90 = torch.tensor([0.7071068, 0.0, 0.0, 0.7071068])
  ...
  mock_command.body_pos_w[:, 0, 0] = 1.0
  mock_command.robot_body_pos_w = mock_command.body_pos_w.clone()

  err_1 = compute_yaw_aligned_root_relative_mpkpe(mock_command)

  mock_command.anchor_quat_w = yaw90.expand(mock_command.num_envs, 4).clone()
  mock_command.robot_anchor_quat_w = yaw90.expand(mock_command.num_envs, 4).clone()
  mock_command.body_pos_w[:, 0, :] = torch.tensor([0.0, 1.0, 0.0])
  mock_command.robot_body_pos_w = mock_command.body_pos_w.clone()

  err_2 = compute_yaw_aligned_root_relative_mpkpe(mock_command)

  assert torch.allclose(err_1, torch.zeros(mock_command.num_envs), atol=1e-5)
  assert torch.allclose(err_2, torch.zeros(mock_command.num_envs), atol=1e-5)
