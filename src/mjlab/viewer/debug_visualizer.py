...
class DebugVisualizer(ABC):
  ...
  def add_box(
    ...
    """Add an axis-oriented box (MuJoCo-style half-extents).

    Args:
      center: Box center in world frame (3D).
      half_size: Half-lengths along each local axis (3D), matching ``mjGEOM_BOX`` ``size``.
      mat: 3×3 rotation from local to world (columns = local basis in world coords).
      color: RGBA (0–1).
      label: Optional label (implementation-specific).
    """
    ...
...
class NullDebugVisualizer:
  ...
  def add_box(self, center, half_size, mat, color, label=None) -> None:
    pass
