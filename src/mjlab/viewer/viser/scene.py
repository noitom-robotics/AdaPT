...
class MjlabViserScene(ViserMujocoScene, DebugVisualizer):
  ...
  Adds debug primitives (arrows, ghosts, spheres, cylinders, boxes, ellipsoids,
  ...
  def __init__(
    ...
    self._boxes = _BatchedPrimitive(
      "boxes",
      lambda: trimesh.creation.box(extents=np.array([2.0, 2.0, 2.0], dtype=np.float64)),
    )
  ...
  def add_box(
    self,
    center: np.ndarray | torch.Tensor,
    half_size: np.ndarray | torch.Tensor,
    ...
    self._queued_boxes.append(
      (
        ...
        np.asarray(_to_numpy(half_size), dtype=np.float32).copy(),
  ...
  def _sync_boxes(self) -> None:
    ...
    colors = np.zeros((n, 3), dtype=np.uint8)
    opacity = 1.0
    for i, (center, half_size, mat, color) in enumerate(self._queued_boxes):
      positions[i] = center + self._scene_offset
