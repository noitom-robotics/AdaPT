...
class Entity:
  ...
  def set_joint_position_target(
    ...
    if isinstance(env_ids, torch.Tensor) and isinstance(joint_ids, torch.Tensor):
      self._data.joint_pos_target[env_ids[:, None], joint_ids[None, :]] = position
      return
  ...
  def set_joint_velocity_target(
    ...
    if joint_ids is None:
      joint_ids = slice(None)
    if isinstance(env_ids, torch.Tensor) and isinstance(joint_ids, torch.Tensor):
      self._data.joint_vel_target[env_ids[:, None], joint_ids[None, :]] = velocity
      return
  ...
  def set_joint_effort_target(
    ...
    if joint_ids is None:
      joint_ids = slice(None)
    if isinstance(env_ids, torch.Tensor) and isinstance(joint_ids, torch.Tensor):
      self._data.joint_effort_target[env_ids[:, None], joint_ids[None, :]] = effort
      return
