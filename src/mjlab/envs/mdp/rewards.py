...
def action_rate_l2(
  ...
  exclude_joint_names: tuple[str, ...] | None = None,
) -> torch.Tensor:
  """Penalize the rate of change of the actions using L2 squared kernel.

  Operates on raw policy output (before per-term scale/offset).
  When *exclude_joint_names* is set, dims for those joints within *action_name*
  (defaults to the first active action term) are omitted from the penalty.
  """
  action_delta = env.action_manager.action - env.action_manager.prev_action
  if not exclude_joint_names:
  ...
  keep_local = [
    i for i, name in enumerate(term.target_names) if name not in exclude_set
  ]
  if not keep_local:
    return torch.zeros(env.num_envs, device=env.device)
  ...
  keep_global = torch.tensor(
    [offset + i for i in keep_local], device=env.device, dtype=torch.long
  )
  delta = action_delta[:, keep_global]
