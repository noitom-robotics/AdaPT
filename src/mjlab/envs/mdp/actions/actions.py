...
def write_joint_position_actuator_targets(
  entity: Entity,
  joint_ids: torch.Tensor,
  processed_joint_positions: torch.Tensor,
) -> None:
  """Write joint position PD targets after encoder compensation.

  ``processed_joint_positions`` are the commanded absolute joint positions (already
  including default pose offset if used). Matches :class:`JointPositionAction`
  and the tennis task ``TrackerToJointPosAction`` (same subtraction + set call).
  """
  encoder_bias = entity.data.encoder_bias[:, joint_ids]
  target = processed_joint_positions - encoder_bias
  entity.set_joint_position_target(target, joint_ids=joint_ids)
