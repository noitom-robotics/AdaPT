...
class ManagerBasedRlEnvCfg:
  ...
  event_manager_cls: type[EventManager] | None = None
  """Optional override for event manager class (task-specific behavior)."""
  ...
  scale_rewards_by_motion_dt: bool = False
  """When True, multiply each reward term by ``pending_dt / fixed_dt`` (per env) from
  the motion command :attr:`motion_reward_dt_command_name`, if that term exists and
  has ``dynamic_dt_enabled``. Keeps cumulative tracking rewards comparable when the
  policy chooses a smaller motion-timeline step than ``fixed_dt``.

  When PPO uses parametric GAE (``extras["gae_step_scale"]`` with
  ``∫_0^Δ γ^x dx`` on rewards), leave this **False** to avoid double-scaling; the
  normalized step size is still exported for GAE via :meth:`_motion_timeline_normalized_dt`.
  """

  motion_reward_dt_command_name: str = "motion"
  """Command term used for :attr:`scale_rewards_by_motion_dt` (expects ``_pending_dt`` / ``_fixed_dt``)."""

  adadt_reward_groups: Any | None = None
  """Optional dual-critic reward grouping (unused in AdaPT Tennis stage-1)."""

  def to_dict(self) -> dict[str, Any]:
    """W&B / file logging: nested dict without deep-copying MuJoCo specs or runtime handles.

    rsl_rl's ``WandbSummaryWriter.store_config`` calls ``env_cfg.to_dict()``; the
    fallback ``dataclasses.asdict`` fails after env build (e.g. tennis latent pipeline).
    """
    from mjlab.utils.dataclass_serialization import asdict_env_cfg_for_yaml

    return asdict_env_cfg_for_yaml(self)
...
class ManagerBasedRlEnv:
  ...
  def load_managers(self) -> None:
    ...
    event_manager_cls = self.cfg.event_manager_cls or EventManager
    self.event_manager = event_manager_cls(self.cfg.events, self)
    ...
    reward_term_heads = None
    groups_cfg = self.cfg.adadt_reward_groups
    if groups_cfg is not None:
      reward_term_heads = groups_cfg.build_term_head_map(self.cfg.rewards.keys())
    self.reward_manager = RewardManager(
      self.cfg.rewards,
      self,
      scale_by_dt=self.cfg.scale_rewards_by_dt,
      reward_term_heads=reward_term_heads,
    )
  ...
  def step(self, action: torch.Tensor) -> types.VecEnvStepReturn:
    ...
    *before* integration, so after stepping, derived quantities (``xpos``,
    ...
    for _ in range(self.cfg.decimation):
      ...
      self.command_manager.compute_substep(self.physics_dt)
    ...
    # Snapshot before ``command_manager.compute``; ``MotionCommand._update_command`` resets
    # ``_pending_dt`` to ``_fixed_dt`` after advancing the timeline.
    gae_motion_scale = self._motion_timeline_normalized_dt()
    self.reward_buf = self.reward_manager.compute(
      dt=self.step_dt,
      per_env_extra_scale=self._motion_dt_reward_per_env_scale(),
    )
    ...
    # Dynamic motion ``dt`` batch mean (set in ``MotionTimeAction.process_actions``).
    # Must run after ``_reset_idx`` repopulates ``extras["log"]`` so W/B step metrics survive resets.
    self._merge_motion_dt_log_metric()

    # Per-env ``pending_dt / fixed_dt`` for parametric GAE (must use pre-compute snapshot).
    if gae_motion_scale is not None:
      self.extras["gae_step_scale"] = gae_motion_scale.unsqueeze(-1).contiguous()
    else:
      self.extras.pop("gae_step_scale", None)

    if self.reward_manager.num_reward_heads == 2:
      self.extras["adadt_reward_groups"] = self.reward_manager.group_reward_buf.clone()
    else:
      self.extras.pop("adadt_reward_groups", None)
  ...
  def _motion_timeline_normalized_dt(self) -> torch.Tensor | None:
    """``pending_dt / fixed_dt`` per env (Δ for parametric GAE), or ``None``."""
    get_term = getattr(self.command_manager, "get_term", None)
    if not callable(get_term):
      return None
    try:
      term = get_term(self.cfg.motion_reward_dt_command_name)
    except KeyError:
      return None
    cfg = getattr(term, "cfg", None)
    if cfg is None or not getattr(cfg, "dynamic_dt_enabled", False):
      return None
    scaler = getattr(term, "reward_timeline_scale", None)
    if callable(scaler):
      return scaler()
    pending = getattr(term, "_pending_dt", None)
    fixed = getattr(term, "_fixed_dt", None)
    if pending is None or fixed is None:
      return None
    return pending / max(float(fixed), 1e-8)

  def _motion_dt_reward_per_env_scale(self) -> torch.Tensor | None:
    """Optional reward multiplier; ``None`` when :attr:`scale_rewards_by_motion_dt` is False."""
    if not self.cfg.scale_rewards_by_motion_dt:
      return None
    return self._motion_timeline_normalized_dt()
  ...
  def _merge_motion_dt_log_metric(self) -> None:
    """Copy motion-timeline metrics into ``extras`` for rsl_rl / W&B."""
    get_term = getattr(self.command_manager, "get_term", None)
    if not callable(get_term):
      return
    try:
      term = get_term("motion")
    except KeyError:
      return
    self.extras.setdefault("log", {})
    if int(getattr(term.cfg, "motion_warmup_steps", 0)) > 0:
      active = float(getattr(term, "is_motion_warmup_active", lambda: False)())
      self.extras["log"]["Metrics/motion_warmup_active"] = active
    if getattr(term.cfg, "dynamic_dt_enabled", False) is not True:
      return
    value = getattr(term, "_wandb_log_dt_mean", None)
    if value is None:
      return
    self.extras["log"]["Metrics/motion_dt_mean"] = value
    active_frac = getattr(term, "_wandb_log_dt_policy_active_frac", None)
    if active_frac is not None:
      self.extras["log"]["Metrics/motion_dt_policy_active_frac"] = active_frac
