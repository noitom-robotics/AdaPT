...
class MjlabOnPolicyRunner(OnPolicyRunner):
  ...
  def load(
    ...
    3. Drop unused ``TwoStageMLPModel`` buffers (e.g. ``residual_scale_tensor``)
      so old mjlab-tennis stage-1 checkpoints load into plain ``MLPModel``.
    """
    ...
    # mjlab-tennis ``TwoStageMLPModel(stage=1)`` always saved this unused buffer.
    # AdaPT Tennis uses plain ``MLPModel``; drop it so strict load still works.
    current_actor_keys = set(self.alg.actor.state_dict().keys())
    leftover = [
      k
      for k in ("residual_scale_tensor",)
      if k in actor_sd and k not in current_actor_keys
    ]
    for k in leftover:
      actor_sd.pop(k)
    if leftover:
      print(
        "[INFO] Dropped leftover TwoStageMLPModel keys not in current actor: "
        f"{leftover}"
      )
    ...
    # Only restore env curricula counters on a true training resume. Play/inference
    # loads with ``load_cfg={"actor": True}`` (iteration=False); restoring a large
    # ``common_step_counter`` from the checkpoint would skip motion warmup with
    # ``motion_warmup_step_source="env"``.
