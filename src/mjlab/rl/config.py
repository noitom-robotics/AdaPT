...
class RslRlModelCfg:
  ...
  """Extra keyword arguments forwarded to the resolved model class.

  This is used by task-specific model extensions (e.g., two-stage residual
  policies) without changing the common runner interface.
  """
...
class RslRlPpoAlgorithmCfg:
  ...
  """L2 penalty on stage-2 residual head output (``mean(residual**2)``). Zero disables."""
  residual_loss_joint_only: bool = True
  """If True, exclude the last action dim (motion ``dt``) from the residual loss."""
  # AMP extensions (used when class_name == "AMPPPO")
  amp_obs_group: str = "critic"
  ...
  amp_discriminator_hidden_dims: tuple[int, ...] = (512, 256)
