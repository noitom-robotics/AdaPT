...
def motion_reached_last_frame(
  env: ManagerBasedRlEnv,
  command_name: str = "motion",
  warmdown_s: float = 0.0,
) -> torch.Tensor:
  """Terminate when motion reaches last frame plus optional warmdown seconds."""
  command = cast(MotionCommand, env.command_manager.get_term(command_name))
  last_idx = torch.clamp(command.motion.lengths[command.motion_ids] - 1, min=0)
  if warmdown_s <= 0.0:
    return command.time_steps_progress >= last_idx
  fps = torch.clamp(command.motion.fps_values[command.motion_ids], min=1.0)
  warmdown_steps = torch.ceil(torch.full_like(fps, float(warmdown_s)) * fps).to(
    dtype=last_idx.dtype
  )
  return command.time_steps_progress >= (last_idx + warmdown_steps)
