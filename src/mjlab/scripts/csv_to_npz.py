  ...
  while not file_saved:
    ...
    if not file_saved:
      ...
      if reset_flag and not file_saved:
        ...
        # Self-describing npz: persist the robot's joint / body name ordering so
        # ``MotionLoader`` can map columns by name even when the runtime robot
        # XML differs from the one used here.
...
def main(
  ...
  Args:
    ...
    g1_dof_layout: ``27`` / ``29`` both use the same 27 driven joints (stock ``g1.xml`` has
      no waist roll/pitch hinges; those links are rigidly attached).
  """
