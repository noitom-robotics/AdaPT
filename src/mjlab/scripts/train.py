...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  if cfg.racket_hand is not None:
    ...
    apply_racket_hand(cfg.env, cfg.racket_hand)
  ...
  motion_cmd = cfg.env.commands.get("motion") if cfg.env.commands is not None else None
  is_tracking_task = (
  ...
  if is_tracking_task:
    ...
    # Local motion: single file, explicit list, or directory (resolved in MotionCommand).
    if motion_cmd.motion_files:
      for p in motion_cmd.motion_files:
        if not Path(p).expanduser().is_file():
          raise FileNotFoundError(f"motion_files entry not found: {p}")
      print(f"[INFO] Using {len(motion_cmd.motion_files)} local motion file(s) (multi-clip).")
    ...
    elif motion_cmd.motion_directory:
      ...
      raise ValueError(
        ...
        "  --env.commands.motion.motion-file /path/to/motion.npz (single local clip)\n"
        "  --env.commands.motion.motion-directory /path/to/dir (all *.npz, multi-clip)\n"
        "  --env.commands.motion.motion-files /a.npz /b.npz ... (explicit list, multi-clip)"
      )
  ...
  if cfg.motion_warmup_steps is not None:
    apply_motion_reference_warmup(
      ...
      step_source=cfg.motion_warmup_step_source or "env",
    )
  ...
  env_cfg = asdict_env_cfg_for_yaml(cfg.env)
