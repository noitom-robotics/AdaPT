...
class MLPModel(nn.Module):
    ...
    def get_latent(
        ...
        max_flat_idx = int(torch.argmax(flat).item())
        min_flat_idx = int(torch.argmin(flat).item())
        max_batch_idx, max_feat_idx = divmod(max_flat_idx, feat_dim)
        min_batch_idx, min_feat_idx = divmod(min_flat_idx, feat_dim)

        group_spans: list[tuple[str, int, int]] = []
        start = 0
        for obs_group, obs_tensor in zip(self.obs_groups, obs_list):
            end = start + obs_tensor.shape[-1]
            group_spans.append((obs_group, start, end))
            start = end

        def _locate_group(global_feat_idx: int) -> tuple[str, int]:
            for group_name, g_start, g_end in group_spans:
                if g_start <= global_feat_idx < g_end:
                    return group_name, global_feat_idx - g_start
            return "unknown", global_feat_idx

        max_group, max_local_dim = _locate_group(max_feat_idx)
        min_group, min_local_dim = _locate_group(min_feat_idx)
        # print(
        #     f"[latent] max={flat[max_flat_idx].item():.6f} "
        #     f"global_dim={max_feat_idx} group={max_group} local_dim={max_local_dim} batch_idx={max_batch_idx}"
        # )
        # print(
        #     f"[latent] min={flat[min_flat_idx].item():.6f} "
        #     f"global_dim={min_feat_idx} group={min_group} local_dim={min_local_dim} batch_idx={min_batch_idx}"
        # )
