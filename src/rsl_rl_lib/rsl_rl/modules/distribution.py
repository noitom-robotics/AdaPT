...
class GaussianDistribution(Distribution):
    ...
    def __init__(
        ...
        elif std_type == 'fixed_scaler':
            self.std_param = torch.ones(output_dim) * init_std
        else:
    ...
    def update(self, mlp_output: torch.Tensor) -> None:
        ...
        elif self.std_type == 'fixed_scaler':
            std = self.std_param.to(mean.device).expand_as(mean)
