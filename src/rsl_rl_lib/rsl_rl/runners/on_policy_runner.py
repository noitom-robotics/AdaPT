...
class OnPolicyRunner:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        for it in range(start_it, total_it):
            # Expose training iteration on env for curriculum/event logic.
            setattr(self.env.unwrapped, "current_learning_iteration", int(it))
            start = time.time()
            # Rollout
            with torch.inference_mode():
                for _ in range(self.cfg["num_steps_per_env"]):
                    ...
                    if hasattr(policy, "get_soft_stage2_actions"):
                        cached = policy.get_soft_stage2_actions()
                        if cached is not None:
                            stage1_action, stage2_action = cached
                            env_unwrapped = self.env.unwrapped
                            setattr(env_unwrapped, "_soft_stage2_stage1_action", stage1_action.to(self.env.device))
                            setattr(env_unwrapped, "_soft_stage2_stage2_action", stage2_action.to(self.env.device))
