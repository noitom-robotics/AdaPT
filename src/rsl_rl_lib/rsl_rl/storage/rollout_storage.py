...
class RolloutStorage:
    ...
    class Transition:
        ...
        def __init__(self) -> None:
            ...
            # Variable-step GAE: ``pending_dt / fixed_dt`` per env (1.0 when unused).
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if self.training_type == "rl":
            ...
            if getattr(transition, "gae_step_scale", None) is not None:
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator[Batch, None, None]:
        ...
        def _print_nonfinite_tensor(name: str, tensor: torch.Tensor, epoch: int, mb_idx: int) -> None:
            if tensor.numel() == 0:
                return
            ...
            if not torch.any(mask):
                return
            if tensor.ndim == 0:
                print(f"[RolloutStorage] epoch={epoch} mb={mb_idx} non-finite in {name} (scalar)")
                return
            flat = mask.reshape(tensor.shape[0], -1)
            bad_ids = torch.nonzero(flat.any(dim=1), as_tuple=False).squeeze(-1)
            show = bad_ids[:32].detach().cpu().tolist()
            print(
                f"[RolloutStorage] epoch={epoch} mb={mb_idx} non-finite in {name}, "
                f"indices={show}, count={int(bad_ids.numel())}"
            )

        def _print_nonfinite_observations(obs: TensorDict, epoch: int, mb_idx: int) -> None:
            for k, v in obs.items():
                if isinstance(v, torch.Tensor):
                    _print_nonfinite_tensor(f"observations.{k}", v, epoch, mb_idx)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                val_batch = values[batch_idx]
                adv_batch = advantages[batch_idx]
                ret_batch = returns[batch_idx]
                logp_batch = old_actions_log_prob[batch_idx]
                dist_batch = tuple(p[batch_idx] for p in old_distribution_params)

                _print_nonfinite_observations(obs_batch, epoch, i)
                _print_nonfinite_tensor("actions", act_batch, epoch, i)
                _print_nonfinite_tensor("values", val_batch, epoch, i)
                _print_nonfinite_tensor("advantages", adv_batch, epoch, i)
                _print_nonfinite_tensor("returns", ret_batch, epoch, i)
                _print_nonfinite_tensor("old_actions_log_prob", logp_batch, epoch, i)
                for j, p in enumerate(dist_batch):
                    _print_nonfinite_tensor(f"old_distribution_params[{j}]", p, epoch, i)
                ...
                yield RolloutStorage.Batch(
                    ...
                    returns=ret_batch,
                    old_actions_log_prob=logp_batch,
                    old_distribution_params=dist_batch,
                )
