...
class TestParametricGAE:
    """Parametric GAE uses ``∫_0^Δ γ^x dx`` on rewards when ``gae_step_scale`` (Δ) varies."""

    def test_gamma_one_reward_coef_equals_delta(self) -> None:
        dt = torch.tensor([0.5])
        coef = PPO._gamma_discount_integral(1.0, dt)
        assert torch.allclose(coef, dt)

    def test_compute_returns_uses_integral_on_rewards(self) -> None:
        num_envs, num_steps = 1, 1
        gamma, lam = 0.99, 0.95
        obs = make_obs(num_envs, OBS_DIM)
        obs_groups = {"actor": ["policy"], "critic": ["policy"]}
        actor = _make_actor(obs, obs_groups, NUM_ACTIONS)
        critic = _make_critic(obs, obs_groups)
        storage = RolloutStorage("rl", num_envs, num_steps, obs, [NUM_ACTIONS])
        ppo = PPO(actor, critic, storage, gamma=gamma, lam=lam, schedule="fixed", normalize_advantage_per_mini_batch=True)

        t = RolloutStorage.Transition()
        t.observations = obs
        t.hidden_states = (None, None)
        t.actions = torch.randn(num_envs, NUM_ACTIONS)
        t.values = torch.tensor([[0.5]])
        t.actions_log_prob = torch.zeros(num_envs)
        t.distribution_params = (torch.zeros(num_envs, NUM_ACTIONS), torch.ones(num_envs, NUM_ACTIONS))
        t.rewards = torch.tensor([1.0])
        t.dones = torch.tensor([0.0])
        t.gae_step_scale = torch.tensor([[0.5]])
        storage.add_transition(t)
        ...
        s = 0.5
        t_coef = (gamma**s - 1.0) / math.log(gamma)
        expected_adv = t_coef * 1.0 + gamma**s * 2.0 - 0.5
        assert torch.allclose(storage.returns[0, 0, 0], torch.tensor(expected_adv + 0.5), atol=1e-4)
